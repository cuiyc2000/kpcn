...
class SpatialGraphConv(MessagePassing):
    def __init__(self, coors, in_channels, out_channels, hidden_size=64, dropout=0):
        """
    ...
    def forward(self, x):
        ...
        """
        shape = x.shape
        edge_index, pos = grid(shape[2], shape[2], device=self.device)
        num_features = shape[1]
        x = x.permute(0, 3, 1, 2).contiguous().view(-1, num_features)
        # edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))  # num_edges = num_edges + num_nodes

        self.propagate(edge_index=edge_index, x=x, pos=pos, aggr='add') # [N, out_channels, label_dim]
